...
function set_count($file_name = 'counter.txt') {
	if ( file_exists($file_name) ) {
		// read the value and increment it by 1
...
function set_count($file_name = 'counter.txt') {
	if ( file_exists($file_name) ) {
		$count = (int) file_get_contents($file_name) + 1;
		// method 4: for small databases or ways to group info together
		// it separates each important data into its key and value
